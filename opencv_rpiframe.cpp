...
std::string gstreamer_pipeline_writer (int capture_width, int capture_height, int display_width, int display_height, int framerate, int flip_method) {
    return "nvarguscamerasrc ! video/x-raw(memory:NVMM), width=(int)" + std::to_string(capture_width) + ", height=(int)" +
           ...
           std::to_string(display_height) + ", format=(string)BGRx ! \
					 ...
					 tune=zerolatency ! h264parse ! rtph264pay ! udpsink host=localhost port=5000";
}

std::string gstreamer_pipeline_capture (int capture_width, int capture_height, int display_width, int display_height, int framerate, int flip_method) {
    return "nvarguscamerasrc ! video/x-raw(memory:NVMM), width=(int)" + std::to_string(capture_width) + ", height=(int)" +
           std::to_string(capture_height) + ", framerate=(fraction)" + std::to_string(framerate) +
           "/1 ! nvvidconv flip-method=" + std::to_string(flip_method) + " ! video/x-raw, width=(int)" + std::to_string(display_width) + ", height=(int)" +
           std::to_string(display_height) + ", format=(string)BGRx ! \
...
int main(int, char**)
{
  ...
  std::string writer_pipeline = gstreamer_pipeline_writer(width,
    height,
    width,
    height,
    framerate,
