...
int main()
{
  ...
  VideoCapture cap ("udpsrc port=5200 ! application/x-rtp, media=video, \
		clock-rate=90000 ! payload=127 ! rtpjpegdepay ! jpegdec ! \
		videoconvert ! autovideosink", CAP_GSTREAMER);
  // VideoCapture cap ("udpsrc port=5200 ! application/x-rtp, media=video, \
		clock-rate=90000, payload=96 ! rtpjpegdepay ! jpegdec ! videoconvert ! \
		autovideosink", CAP_GSTREAMER);
